...
KNOWN ISSUES
- This script is designed for only one match to be active at a time.
  There are issues with multiple matches being active at once.
'''
...
# discord webhook URL
url = "https://discord.com/api/webhooks/ENTER HERE"
...
while True:  # forever loop, until match is over...
    ...
    try:
        ...
        for ID in ids:
                ...
                if not oldStates[ID][i] == states[ID][i]:
                    match oldStates[ID][i]:
                        case "called":
                            data["embeds"] = [
                                {
                                    ...
                                    "fields": [
                                        {
                                            'name': getPlayer(p1Id, players)['name'] + " (" +
                                                    str(getPlayer(p1Id, players)['wins']) + "-" +
                                                    str(getPlayer(p1Id, players)['losses']) + ") vs " +
                                                    getPlayer(p2Id, players)['name'] + " (" +
                                                    str(getPlayer(p2Id, players)['wins']) + "-" +
                                                    str(getPlayer(p2Id, players)['losses']) + ")",
                                            "value": ""
                                        }
                                    ]
                                }
                            ...
                            logIt(
                                "Match Called (" + tourney['title'] + " - " + games[i]['name'] + "): " +
                                getPlayer(p1Id, players)['name'] + " vs " + getPlayer(p2Id, players)['name']
                            )
                        ...
                        case "active":
                            ...
                            result = requests.post(url, json=data)
                            logIt(
                                "Match Active (" + tourney['title'] + " - " + games[i]['name'] + "): " +
                                getPlayer(p1Id, players)['name'] + " vs " + getPlayer(p2Id, players)['name']
                            )
                            jsonData["showPlayers"] = True
                            jsonData["showWinner"] = False
                            jsonData['p1name'] = (getPlayer(p1Id, players)['name'] + " (" +
                                                  str(getPlayer(p1Id, players)['wins']) + "-" + str(
                                                  getPlayer(p1Id, players)['losses']) + ")")
                            jsonData["p1icon"] = getPlayer(p1Id, players)['photoUrl']
                            jsonData['p1shortName'] = getPlayer(p1Id, players)['name']
                            jsonData["p2name"] = (getPlayer(p2Id, players)['name'] + " (" +
                                                  str(getPlayer(p2Id, players)['wins']) + "-" + str(
                                                  getPlayer(p2Id, players)['losses']) + ")")
                        ...
                        case "done":
                            ...
                            data["embeds"] = [
                                {
                                    ...
                                    "fields": [
                                        {'name': getPlayer(p1Id, players)['name'] + " (" +
                                                 ...
                                                 getPlayer(p2Id, players)['name'] + " (" +
                                                 str(getPlayer(p2Id, players)['wins']) + "-" +
                                                 str(getPlayer(p2Id, players)['losses']) + ")", "value": ""},
                            ...
                            result = requests.post(url, json=data)
                            logIt(
                                "Match Done (" + tourney['title'] + " - " + games[i]['name'] + "): " +
                                getPlayer(p1Id, players)['name'] + " vs " + getPlayer(p2Id, players)['name'] + ", " +
                                getPlayer(wid, players)['name'] + " Wins"
                            )
                            if jsonData["p1shortName"] == getPlayer(p1Id, players)['name'] and jsonData[
                                "p2shortName"] == getPlayer(p2Id, players)['name']:
                            ...
                            scroller.append(
                                getPlayer(p1Id, players)['name'] + " vs " + getPlayer(p2Id, players)['name'] + " = " +
                                getPlayer(wid, players)['name'] + " WINS!")
                            jsonData["scroller"] = "Recent Matches: " + scroller[0] + " | " + scroller[1] + " | " + \
                                                   scroller[2]
            ...
            winners = [0, 0, 0]
            # print(tourney['title'] + ": " + str(tourney['endTime']))
            if (not tourney['endTime'] is None) and (finished[ID] == 0):  # tourney is complete
                finished[ID] = 1
                ...
                data["embeds"] = [
                    {
                        ...
                        "fields": [
                            {
                                'name': "First Place:",
                                'value': getPlayer(winners[0], players)['name'] + " (" +
                                         str(getPlayer(winners[0], players)['wins']) + "-" +
                                         str(getPlayer(winners[0], players)['losses']) + ") "
                            },
                            {
                                'name': "Second Place:",
                                'value': getPlayer(winners[1], players)['name'] + " (" +
                                         str(getPlayer(winners[1], players)['wins']) + "-" +
                                         str(getPlayer(winners[1], players)['losses']) + ") "
                            },
                            {
                                'name': "Third Place:",
                                'value': getPlayer(winners[2], players)['name'] + " (" +
                                         str(getPlayer(winners[2], players)['wins']) + "-" +
                                         str(getPlayer(winners[2], players)['losses']) + ") "
                            },
                        ]
                    }
                ]
                result = requests.post(url, json=data)
                logIt(
                    "Tourney Done (" + tourney['title'] + "): 1st: " + getPlayer(winners[0], players)['name'] +
                    ", 2nd: " + getPlayer(winners[1], players)['name'] +
                    ", 3rd: " + getPlayer(winners[2], players)['name']
                )
